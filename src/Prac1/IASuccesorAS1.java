...
public class IASuccesorAS1 implements SuccessorFunction{
    public List getSuccessors(Object state) {
       ...
       for(int cd1 = 0; cd1 < board.mapLength();++cd1){
            for(int p = 0; p < board.petLength(); ++p) {
                ...
                if (tmp.AddViaje(cd1, p)) retval.add(new Successor("" + tmp.getPetAtendidas(), tmp));
                ...
                for(int i2 = i1; i2 < board.mapLength(); ++i2){
                    for(int j2 = 0; j2 < board.sizeViajes(i2); ++j2){
                        if(i1 == i2 && (j2 <= j1 || (j1%2 == 0 && j2 == j1+1)));
                            ...
                            tmp = board.copyState();
                            if(tmp.SwapViaje(i1,j1,i2,j2)) retval.add(new Successor("" + tmp.getPetAtendidas(), tmp));
